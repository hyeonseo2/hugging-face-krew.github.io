...
AI 에이전트는 API 호출, 데이터 처리, 복잡한 문제 해결 등 실제 세계에서 액션을 취할 수 있어야 합니다. 에이전트가 이러한 액션을 표현하는 방식은 여러 패러다임을 거쳐 발전했습니다.
...
* **경직된 구조**: 제공된 도구가 실제로 필요한 작업을 정확히 대응하지 못하는 경우 유연하게 대처하기 어려움
...
* **OpenAI 모델**: 구조화된 접근법 적용 시, 가장 큰 성능 향상을 보이며 특히 추론이 중요한 작업에서 큰 향상
* **Claude 모델**: 전반적으로 구조화의 이점을 보였으며, 특히 Claude 3.7 Sonnet에서 큰 성능 향상을 보임.
...
구조화된 생성의 중요성을 확인하기 위해, 우리는 벤치마크 전반에서 15,724개의 에이전트 트레이스를 분석했습니다. 그 결과는 매우 인상적이었습니다:

* 전체 트레이스 중 **2.4%**는 첫 호출 시 파싱 오류 발생
* 첫 호출에 **파싱 오류가 있는** 경우 성공률: **42.3%**
* 첫 호출에 **파싱 오류가 없는** 경우 성공률: **51.3%**

**첫 호출에서 파싱 오류가 없는 에이전트 트레이스는, 있는 경우보다 성공률이 21.3% 더 높았습니다**
...
* 에이전트 출력에 대해 신회할 수 있는 파싱이 필요할 때
...
추론 과정을 더 명시적으로 표현하는 것은 모델이 문제 해결 경로를 더 잘 유지할 수도록 돕거나, 혹은 단순히 파싱이 쉬워서일 수도 있습니다. 어느 쪽이든 좋습니다.
